...
function displayPlaces(places) {
    ...
    for ( var i=0; i<places.length; i++ ) {
        ...
        var placePosition = new kakao.maps.LatLng(places[i].y, places[i].x),
            keywordMarker = addMarker(placePosition, i), 
        ...
        (function(keywordMarker, title) {
            // 키워드로 검색된 마커에 클릭이벤트를 등록, 클릭시 해당 마커의 주소가 input 태그에 입력
        ...
        (function(keywordMarker, title) {
            kakao.maps.event.addListener(keywordMarker, 'mouseover', function() {
                displayInfowindow(keywordMarker, title);
            });
...
function addMarker(position, idx, title) {
    var imageSrc = 'https://t1.daumcdn.net/localimg/localimages/07/mapapidoc/marker_number_blue.png', // 마커 이미지 url, 스프라이트 이미지를 씁니다
        ...
        keywordMarker = new kakao.maps.Marker({
            position: position, // 마커의 위치
            image: markerImage 
        });

    keywordMarker.setMap(map); // 지도 위에 마커를 표출합니다
    keywordMarkers.push(keywordMarker);  // 배열에 생성된 마커를 추가합니다

    return keywordMarker;
}
...
// 인포윈도우에 장소명을 표시합니다
function displayInfowindow(keywordMarker, title) {
