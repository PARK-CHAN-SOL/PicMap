    ...
    for (const mutation of mutationList) {
        if (mutation.type === "attributes") {
            if(mypageObserverTargetSave.style.display=='none' && pillsFriends.classList.length == 4){
                ...
                travelObserverTarget.style.display='block';
...
const travelObserverSave = new IntersectionObserver((items)=>{ // IntersectionObserver를 생성, items는 관찰된 요소의 배열
    items.forEach(async (item)=>{ // 관찰된 각 요소에 대해 반복
        ...
        if(mypageObserverTargetSave.dataset.startRow <= travelTotalCount){ // startRow가 travelTotalCount보다 작거나 같을 때만 실행
...
async function makeTravelListSave (url, formData) {
    try{
        ...
        mypageObserverTargetSave.dataset.startRow = parseInt(mypageObserverTargetSave.dataset.startRow)+18; // startRow 값을 9 증가
        mypageObserverTargetSave.dataset.endRow = parseInt(mypageObserverTargetSave.dataset.endRow)+18; // endRow 값을 9 증가
